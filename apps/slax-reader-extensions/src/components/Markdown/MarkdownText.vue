...
.markdown-text {
  --primaryTextColor: #ffffffe6;
  --secondaryTextColor: #ffffffcc;
  .markdown-content {
    --style: flex flex-col;
    &::v-deep(*) {
      --style: m-0 $primaryTextColor;
    }

    &::v-deep(h1) {
      --style: font-bold text-(20px $primaryTextColor) line-height-28px 'not-first:mt-32px';
    }

    &::v-deep(h2) {
      --style: font-bold text-(16px $secondaryTextColor) line-height-22px mt-32px;
    }

    &::v-deep(h3),
    &::v-deep(h4),
    &::v-deep(h5) {
      --style: font-bold text-(12px $secondaryTextColor) mt-20px;
    }
    ...
    &::v-deep(ul) {
      ...
      li {
        --style: relative pl-20px box-border font-normal text-(14px #ffffff99) line-height-20px;

        &::marker {
          --style: content-none hidden text-#A8B1CD66;
        }
        ...
        & li {
          ...
          &:before {
            --style: box-border top-8px left-4px w-4px h-4px border border-#A8B1CD66 border-solid bg-#A8B1CD66;
          }
        }
        ...
        &:before {
          --style: absolute top-8px left-4px w-4px h-4px bg-#A8B1CD66 content-empty rounded;
        }
      }
    }

...
import ImportLoadingModal from '#layers/core/app/components/ThirdPartyImport/ImportLoadingModal.vue'
...
const importThirdPartyData = async (type: string) => {
  ...
  if (type === 'omnivore') {
    metadataList = file.files[0] ? await unzipGetFile(file.files[0], /metadata_[0-9]+_to_[0-9]+.json/) : []
  } else if (type === 'pocket') {
...
section {
  ...
  .title {
    --style: font-600 text-(24px txt) line-height-33px text-left select-none;
  }
  ...
  .info {
    .import {
      ...
      .import-description {
        ...
        p {
          --style: text-(14px txt) line-height-22px;
        }
      }

...
export class ArticleSelection {
  ...
  async drawMark(marks: MarkDetail) {
    // 1. 准备数据
    const userListMap = this.createUserListMap(marks.user_list)
    const commentMap = this.buildCommentMap(marks.mark_list, userListMap)
  ...
  // 创建用户列表映射
  private createUserListMap(userList: UserList): Map<number, MarkUserInfo> {
    return new Map(Object.entries(userList).map(([key, value]) => [Number(key), value]))
  }

  // 构建评论映射
  private buildCommentMap(markList: MarkInfo[], userListMap: Map<number, MarkUserInfo>): Map<number, MarkCommentInfo> {
  ...
  // 构建评论之间的关系
  private buildCommentRelationships(markList: MarkInfo[], commentMap: Map<number, MarkCommentInfo>): void {
    for (const mark of markList) {

<template>
  <div class="raw-web">
    ...
    <div class="content-wrapper">
      ...
      <RawWebPanel v-if="inlineBookmarkDetail" ref="rawWebPanel" @is-dragging="val => (isDragging = val)" @selectedTypeUpdate="val => (selectedType = val)">
        <template #ai>
          <Transition name="opacity">
            <div v-show="selectedType === PanelItemType.AI" class="dark px-20px py-4px">
              <AISummaries :share-code="id" :is-appeared="selectedType === PanelItemType.AI" :close-button-hidden="true" :content-selector="'body'" @navigated-text="() => false" />
            </div>
          </Transition>
        </template>
        <template #chat>
          <Transition name="opacity">
            <div v-show="selectedType === PanelItemType.Chat" class="dark size-full">
              <ChatBot :share-code="id" :is-appeared="selectedType === PanelItemType.Chat" :close-button-hidden="true" ref="chatbot" />
            </div>
          </Transition>
        </template>
      </RawWebPanel>
      ...
      showFeedbackView(
        {
          ...
          title: inlineBookmarkDetail.value?.title || t('page.bookmarks_detail.no_title'),
          shareCode: id
        },
        'parse_error'
      )
